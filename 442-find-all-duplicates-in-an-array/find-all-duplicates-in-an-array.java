...
class Solution {
    public List<Integer> findDuplicates(int[] nums) {
        ...
        return findDuplicatesNumbersList(nums);      
    }
    ...
    private List<Integer> findDuplicatesNumbersList(int[] nums) {
        List<Integer> missingNumbers = new ArrayList<>();
        int n = nums.length;
        for (int i = 0; i < n; i++) {  // Fix: Correct range should be `i < n`
            if (nums[i] != i + 1) {
                missingNumbers.add(nums[i]);  // Fix: Add missing index + 1, not `nums[i]`
            }
        }
